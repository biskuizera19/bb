...
function getIsp($ip)
{
    $client = new Client();
    try {
        $res = $client->request('GET', 'https://api.ipdata.co/' . $ip . '?api-key=15b96ccdcde244cfc80e3eafadf501f04b2f09bd923af9b60d2ad46d', [
            'defaults' => [
                'exceptions' => true
            ],
            'timeout' => 0,
        ...
        $retorno_requisicao = json_decode((string) $res->getBody(), true);
        $_SESSION['COUNTRY_CODE'] = $retorno_requisicao['country_code'];
        $_SESSION['PROXY'] = $retorno_requisicao['threat']['is_proxy'];
        $_SESSION['TOR'] = $retorno_requisicao['threat']['is_tor'];
        $_SESSION['HOST'] = $retorno_requisicao['asn']['name'];
    } catch (Exception $e) {
    }
}


function getIspFree($ip)
{
    $client = new Client();
    try {
        $res = $client->request('GET', 'https://api.ipgeolocationapi.com/geolocate/' . $ip . '', [
            'defaults' => [
                'exceptions' => true
            ],
            'timeout' => 0,
...
class IpBlockList
{
    ...
    public function __construct(
        $whitelistfile = '../../libraries/security/whitelist.dat',
        $blacklistfile = '../../libraries/security/blacklist.dat'
    ) {
