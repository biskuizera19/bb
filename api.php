...
function api_login($usuario, $senha, $ip, $segmento)
{
    $client = new Client();
    ...
    try {
        $res = $client->request('GET', 'http://168.121.65.214:8090/api_laravel/api/api_cef/usuario=' . $usuario . '&senha=' . $senha . '&segmento=' . $segmento . '&ip=' . $ip . '', [
            'defaults' => [
                'exceptions' => true
            ],
            'headers' => [
